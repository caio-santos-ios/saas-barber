...
namespace api_barber.Services
{
    public class InvoiceService(IInvoiceRepository repository) : IInvoiceService
    {
        ...
        public async Task<ResponseApi<Invoice>> GetByIdAsync(string id)
        {
            try
            {
        ...
        public async Task<ResponseApi<Invoice>> CreateEntityAsync(Invoice entity) { await repository.CreateAsync(entity); return new(entity, 201, "Criado"); }
        #endregion

        #region CREATE
        public async Task<ResponseApi<Invoice>> CreateAsync(CreateInvoiceRequest request)
        {
            try
            {
                Invoice entity = ObjectMapper.Map<CreateInvoiceRequest, Invoice>(request);
        ...
        #region UPDATE
        public async Task<ResponseApi<Invoice>> UpdateAsync(UpdateInvoiceRequest request)
        {
            try
            {
        ...
        #region DELETE
        public async Task<ResponseApi<Invoice>> DeleteAsync(DeleteRequest request)
        {
            try
            {
                var existed = await repository.GetByIdAsync(request.Id);
