...
namespace api_barber.Services
{
    public class NotificationService(INotificationRepository repository) : INotificationService
    {
        ...
        public async Task<ResponseApi<Notification>> CreateEntityAsync(Notification entity) { await repository.CreateAsync(entity); return new(entity, 201, "Criado"); }
        #endregion
        ...
        public async Task<ResponseApi<Notification>> CreateAsync(CreateNotificationRequest request)
        {
            try
            {
                Notification entity = ObjectMapper.Map<CreateNotificationRequest, Notification>(request);
        ...
        #region UPDATE
        public async Task<ResponseApi<Notification>> UpdateAsync(UpdateNotificationRequest request)
        {
            try
            {
                var existed = await repository.GetByIdAsync(request.Id);
                if(existed is null) return new(null, 404, "Não encontrado");
                Notification entity = ObjectMapper.Map<UpdateNotificationRequest, Notification>(request);
